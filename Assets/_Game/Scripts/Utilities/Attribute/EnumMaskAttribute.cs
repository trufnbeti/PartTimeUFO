...
{
    public static bool IsEnumPositive(int mask, int enumValue)
    {
        return ((mask >> enumValue) & 0x01) == 1;
    }

    public static void SetPositive(ref int mask, int enumValue)
    {
        mask |= (1 << enumValue);
    }
    public static void SetNegative(ref int mask, int enumValue)
    {
        mask &= ~(1 << enumValue);
    }
